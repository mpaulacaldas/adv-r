...
1. What are the four common types of atomic vectors? What are the two 
   ...
   ::: answer
   - Common types: logical, integer, numeric, character.
   - Rare types: raw, complex.
   :::

1. What are attributes? How do you get them and set them?

   ::: answer
   Attributes are "metadata" associated with an object. To get them, we can use `attr()` or `attributes()`. 
   ...
   They can also be used to set attributes.
   ...
   # all at once
   y <- letters[1:5]
   attributes(y) <- list(animal = "frog", number = 100)
   y
   ```
   
   In addition to these two, we can also set attributes when we create an object via `structure()`.
   
   ```{r}
   # set attributes
   structure(1:6, dim = 2:3, foo = "bar")
   ```
   :::
...
1. How is a list different from an atomic vector? How is a matrix different
   ...
   ::: answer
   - Lists can contain elements of different types, atomic vectors require all element to be of the same type.
   - All elements of a matrix need to be of the same type.
   :::
...
1. Can you have a list that is a matrix? Can a data frame have a column 
   ...
   ::: answer
   We can have a list in a matrix provided that all of the elements of the matrix are of type list.
   ```{r}
   matrix(
     c(list(rnorm(3)), list(letters[1:4])), 
     nrow = 2
   )
   ```
   
   Yes, a data frame can have a column that is a matrix.
   :::
   
1. How do tibbles behave differently from data frames?
  
   ::: answer
   - They have a nicer print method.
   - They don't recycle vectors unless they are of length one.
   - They prevent partial matching with `$`.
   - Subestting via `[` always returns a tibble.
   - They don't automatically create factors.
   :::
...
1. How do you create raw and complex scalars? (See `?raw` and 
   ...
   ::: answer
   Raw vectors need to be created from another vector type. Note that `raw()` is used to create an empty raw vector of a given size, e.g. `raw(2)` is equivalent to `vector("raw", 2)`.
   ```{r}
   (rr <- raw(2))          # set the length of the vector
   
   rr[1] <- as.raw(100)    # convert numeric to raw
   rr[2] <- charToRaw("r") # convert character to raw
   
   rr
   ```
   
   Here is a slightly more fun example:
   ```{r}
   cat(hello <- paste("hello", emo::ji("world")))
   (gibberish <- charToRaw(hello))
   dput(gibberish)
   ```
   
   Complex vectors can be created with `complex()`. Here is an example from the documentation:
   
   ```{r}
   (z <- complex(real = stats::rnorm(10), imaginary = stats::rnorm(10)))
   ```
    
   We can also set the `modulus` and `arguement` (i.e. the polar coordinates).
   :::
...
1. Why is `1 == "1"` true? Why is `-1 < FALSE` true? Why is `"one" < 2` false?
   ...
   ::: answer
   According to the documentation of `?Comparison`, if two arguments are atomic vectors of different types, one is coerced to the type of the other.
   
   `1 == "1"` returns true because `1` is coerced to character.
   ```{r}
   1 == "1"
   ```
   
   `-1 < FALSE` returns true because `FALSE` is coerced to `0` (numeric), which is greater than `-1`.
   
   ```{r}
   -1 < FALSE
   ```
   
   `"one" < 2` returns false because `2` is coerced to `"2"` (character), which is not an exact match to the `"one"` string.
...
1. Why is the default missing value, `NA`, a logical vector? What's special
   ...
   ::: answer
   The default `NA` is of type logical so that it can be coerced to other vector types: integer, numeric, complex and character. This is because logical has the lowest order of precedence out of all vector types (except for raw).
   :::

1. Precisely what do `is.atomic()`, `is.numeric()`, and `is.vector()` test for?

   ::: answer
   `is.atomic()` returns true if a vector is of the atomic types and (i.e. not a list).
   ...
   `is.vector()` returns true if a vector is of the specified mode and has no attributes other than names.
   ...
   `is.numeric()` returns true if a vector is of type integer or double (and not a factor) and the values can reasonably be regarded as numeric.
...
1.  What does `dim()` return when applied to a 1-dimensional vector?
    ...
    ::: answer
    `dim()` only works on matrices, arrays or data frames.
    ```{r}
    dim(1:10)
    ```
    :::
    ...
    ::: answer
    `NROW()` and `NCOL()` treat any vector as a 1-column matrix.
    ```{r}
    NROW(1:12)
    NROW(numeric())
    ```
    
    According to the above definition, `NCOL()` will always return 1, so I don't see any realistic situation where I might use it.
    ...
    Unlike `nrow()`, `NROW()` won't return `NULL` when applied to an atomic vector.
...
1.  How would you describe the following three objects? What makes them
    ...
    ::: answer
    In the above expressions, a vector is passed to the `dim` argument, which gives the maximal indices on each direction: rows, columns and other dimensions. The product of these values must match the length of the array.
    ```{r}
    # one row, one column, across 5 dimensions
    x1
    
    # one row, five columns, one dimension
    x2
    
    # five rows, one column, one dimension
    x3
    ```
    :::
...
1.  An early draft used this code to illustrate `structure()`:
    ...
    ::: answer
    This happens because some attribute values, like comment, are treated specially and have restrictions on the values which can be set. 
    
    The comment attribute is never printed by the default method of `print()`. To access it, we need to call the `comment()` function.
...
1.  What sort of object does `table()` return? What is its type? What 
    ...
    ::: answer
    `table()` creates a contingency table of factor levels.
    ```{r}
    (tab <- table(rpois(100, 5)))
    ```
    
    It returns an object of type integer, with dim, dimnames and class attributes. 
    ...
    The dimensionality increases when we provide a list with more than one vector.
    
    ```{r}
    attributes(table(iris))
    ```
    :::
...
1.  What happens to a factor when you modify its levels? 
    ...
    ::: answer
    In the first line, `factor()` maps each unique value of `letters` to an integer value and assigns a label. For example, the value `1` is assigned the label `a`, `2` the label `b`, etc.
    ...
    The second line reverses the mapping of labels to levels: it assigns `z` to `1`, `y` to `2`, and so on. The order of the underlying numerical vector remains unaffected.
...
1.  What does this code do? How do `f2` and `f3` differ from `f1`?
    ...
    ::: answer
    `f2` is a factor vector whose values have been reversed, but its levels remain in alphabetical order. This is because the `levels` attribute is unaffected by any operation on the vector *unless* it is made via the `levels()` function or the `levels` argument of `factor()`.
    ```{r}
    f2
    as.numeric(f2)
    ```
    
    In this case, the levels of `f3` were determined from the get-go as the reverse `letters`. In the integer representation of the factor, the label `a` is encoded as `26`.
    ```{r}
    f3
    as.numeric(f3)
    ```
...
1.  List all the ways that a list differs from an atomic vector.
    ...
    ::: answer
    - The elements of lists can be of any type, not just vectors.
    - Each element of a list is a reference to another object, therefore creating a list does not involve copying the components into the list.
    - Lists can contain other lists. Combining lists with `list()` will create another list; combining vectors with `c()` will create another vector.
    :::
...
1.  Why do you need to use `unlist()` to convert a list to an 
    ...
    ::: answer
    Applying `as.vector()` with the default `mode = "any"` will return the original list.
    ...
    Specifying a `mode` can give very different results.
    
    ```{r, error = TRUE}
    as.vector(l3, mode = "numeric")
    as.vector(l4, mode = "numeric")
    as.vector(l5, mode = "numeric")
    ```
    
    `unlist()` is the only option that "flattens" the list and combines all elements.
...
1.  Compare and contrast `c()` and `unlist()` when combining a 
    ...
    ::: answer
    Date and date-time are vectors of type `"double"` with a specific class that determines how some functions will work.
    ```{r}
    today <- Sys.Date()
    now   <- Sys.time()
    ...
    Combining date and date-time with `unlist()` will get rid of the `"class"` attribute of both vectors.
    ```{r}
    (d1 <- unlist(list(today, now)))
    typeof(d1)
    class(d1)
    ```
    
    `r emo::ji("scream")` Combining date and date-time with `c()` will result in a vector of class date or date-time, depending on the type of the first argument. This is because `c()` will dispatch the generic function `c.Date()` or `c.POSIXct()` depending on the type of the first argument.
    ...
    Note that even though the underlying values don't change with the coercion, the way they are displayed and used by other functions *will*.
    ```{r}
    unclass(d2) == d1
    ```
    
    To avoid this problem, we need to make sure to transform the values explicitely before attempting either one of the methods.
    
    ```{r}
    now <- as.Date(now) # from date-time to date
    
    c(today, now)
    as.Date(unlist(list(today, now)), "1970-01-01")
    ```
...
1.  Can you have a data frame with zero rows? What about zero columns?
    ...
    ::: answer
    Yes, you can have data frames and tibbles with zero columns and zero rows.
    ```{r}
...
1.  What happens if you attempt to set rownames that are not unique?
    ...
    ::: answer
    ```{r, error = TRUE}
    data.frame(x = 1:2, y = 2:1, row.names = c("a", "a"))
...
1.  If `df` is a data frame, what can you say about `t(df)`, and `t(t(df))`? 
    ...
    ::: answer
    `t()` transposes the data frame and returns a matrix. Since all elements of a matrix must be of the same type, the vectors of the data frame are coerced into the type with the highest order of precedence.
    ```{r}
    df1 <- data.frame(
      num = 1:3, 
      chr = letters[1:3],
      ftr = factor(letters[1:3]),
      stringsAsFactors = FALSE
      )
    str(df1)
    
    t(df1) # elements are coerced to character
    t(t(df1))
    ```
    :::
...
1.  What does `as.matrix()` do when applied to a data frame with 
    ...
    Similarly to `t()`, `as.matrix()` coerces all vectors to the type of the vector with the highest order of precedence.
    ```{r}
    as.matrix(df1)
    
    df2 <- data.frame(lgl = c(TRUE, FALSE), dbl = 1:2)
    as.matrix(df2)
    ```
    
    `data.matrix()` converts all variables in a data frame to numeric mode and then binds them together. Note that factors are replaced with their internal codes, and character vectors are coerced to `NA`.
