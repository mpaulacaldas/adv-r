...
1.  What is the result of subsetting a vector with positive integers, 
    ...
    ::: answer
    Positive integers extract the value, negative integers remove the value.
    ```{r}
    letters[1]
    letters[-c(1:15)]
    ```
    
    Subsetting with logical vectors will return the elements marked with `TRUE`.
    ...
    Subsetting with character vectors is possible if the vector is named.
...
1.  What's the difference between `[`, `[[`, and `$` when applied to a list?
    ...
    ::: answer
    `[` returns a list, `$` and `[[` return the element.
    
    ```{r}
    l <- list(x = 1:3, y = c("a", "b"))
...
1.  When should you use `drop = FALSE`?
    ...
    ::: answer
    You should use it to avoid type conversion in data frames.
...
1.  If `x` is a matrix, what does `x[] <- 0` do? How is it different from
    ...
    ::: answer
    `x[] <- 0` will populate the entire matrix with zeroes. `x <- 0` will change the binding of `x` from a matrix to a numeric vector.
    ```{r}
...
1.  How can you use a named vector to relabel categorical variables?
    ...
    ::: answer
    This is how I would do it, but there must be other ways.
    ```{r}
    y   <- letters[1:5]
    new <- c("a" = "apple", "b" = "banana", "c" = "carrot")
...
`r emo::ji("gem")` `outer()` creates a matrix with the product (or the result of `FUN`) of `X` and `Y`. 

`%o%` is a wrapper for `outer(X, Y, "*")`.

```{r}
one_to_five <- 1:5

# multiplication table
one_to_five %o% one_to_five

# power table
outer(one_to_five, one_to_five, "^")
```
:::
...
*   For matrices and arrays, any dimensions with length 1 will be dropped:
    ...
    ::: answer
    This is easier to see without `str()`.
    ```{r}
    a[1, ]
    a[1, , drop = FALSE]
    ```
...
1.  Fix each of the following common data frame subsetting errors:
    ...
    mtcars[mtcars$cyl == 4 | mtcars$cyl == 6, ] # good 
    mtcars[mtcars$cyl %in% c(4, 6), ]           # better
    ```
    :::
...
1.  Why does the following code yield five missing values? (Hint: why is 
    ...
    ::: answer
    The reason is because logical vectors of size 1 are recycled, and `NA` is by default of type logical.
    
    There is no recycling with integer subsetting, which is why `NA_real_` returns only one missing value.
    ```{r}
    is.logical(NA)
    x[NA_real_]
    ```
...
1.  Why does `mtcars[1:20]` return an error? How does it differ from the 
    ...
    `mtcars[1:20]` is subsetting on the columns. It is equivalent to `mtcars[, 1:20]`. The expression returns an error because we are asking for more columns than the ones that are available in the data frame.
    ```{r, error = TRUE}
    mtcars[1:20]
    ncol(mtcars)
    ```
    
    `mtcars[1:20, ]` subsets on the rows of the data. When we ask for more rows that are available in the data, we get back a data frame with extra rows filled with `NA` values.
...
1.  What does `df[is.na(df)] <- 0` do? How does it work?
    ...
    ::: answer
    `df[is.na(df)] <- 0` replaces all missing values with zero. `is.na(df)` returns a logical matrix with `TRUE` for every entry with a missing value in `df`. Subsetting by this matrix allows us to conditionally replace the `NA` values with `0`.
...
```{r, include=FALSE}
# Load back so that I can use it in my solutions
...
::: answer
`r emo::ji("gem")` This is a nice hack, that takes advantage of two behaviours of vector subsetting:

- the returned vector will have the same order as `x`
- new elements will be added to `lookup` if the indexing vector `x` contains duplicates

I already showed my approach with `dplyr::recode()` in the quiz at the beginning. It isn't great for all situation because it takes a dependency on `dplyr` and uses the `!!!` operator, which still causes a lot of confusion. Wrapping Hadley's approach is a nice little alternative. Note however that unlike `dplyr::recode()`, the lookup table approach requires all the alternatives to be named in the lookup vector.

```{r}
recode2 <- function(x, lookup) { unname(lookup[x]) }

frutas <- c("manzana", "fresa", "kiwi")
fruits <- c("manzana" = "apple", "fresa" = "strawberry")

recode2(frutas, fruits)
dplyr::recode(frutas, !!!fruits)
```
...
::: answer
`r emo::ji("gem")` This is also useful for dates.

```{r}
df <- data.frame(
  start = as.Date(c("2000-01-01", "2000-02-01")), 
  end   = as.Date(c("2000-01-03", "2000-02-04"))
  )

(diff <- df$end - df$start + 1)
(idx <- rep(1:nrow(df), diff))
df[idx, ]
```
:::
...
::: answer
Remember: invert the operator and distribute the negation.
:::
...
* When the logical vector contains `NA`, logical subsetting replaces these 
  ...
  People also use `%in%` for this side-effect, which seems just as wrong.
  ```{r}
  x <- c("a", NA, "b")
  x[x == "a"]
  x[x %in% "a"]
...
1.  How would you randomly permute the columns of a data frame? (This is an
    ...
    df[sample(1:nrow(df)), sample(names(df))]
    ```
    :::
...
1.  How would you select a random sample of `m` rows from a data frame? 
    ...
    ::: answer
    Following the same approach as the [solutions manual](https://advanced-r-solutions.rbind.io/subsetting.html#applications):
    
    ```{r}
    m <- 4
    start <- sample(nrow(df) - m + 1, 1)
    end   <- start + m - 1
...
1.  How could you put the columns in a data frame in alphabetical order?
    ...
    cars[sort(names(cars))]
    
    cars[order(names(cars))]
    ```
    :::
