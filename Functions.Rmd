...
1.  We can use `on.exit()` to implement a simple version of `capture.output()`.
    ...
    Unlike what the [solutions manual](https://advanced-r-solutions.rbind.io/functions.html#exiting-a-function) states, `capture.output()` does not reimplement the `readLines()` method. If `file = NULL`, `capture.output()` writes the output to a variable called `rval`. After the output of the expressions have been evaluated, `capture.output()` returns `rval`, which is why we can see the output in the console. Note that if we were to explicitly provide a file path where to store the output, then `capture.output()` will return an invisible `NULL`.
