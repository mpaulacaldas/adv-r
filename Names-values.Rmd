...
1.  I slightly simplified the rules that govern syntactic names. Why is `.123e1`
    ...
    ::: answer
    `.123e1` is not a syntatically valid name because it starts with a dot and is followed by the number.
    :::
...
1.  Predict the output of the following code:
    ...
    When the first element of the list is modified, the size of `b` remains the same as the previous step. Although the two elements of the vector are back to being the same, R doesn't recognise them as a shared value. This also explains why in total, `a` and `b` have the size of three distict values.
...
1.  Wrap the two methods for subtracting medians into two functions, then
    ...
    ::: answer
    In the code below, I create three functions: one that generates random data from a given number of colums and two that implement the median substraction. Note that I also make sure that both median functions output a `data.frame` object. 
    ```{r, error = TRUE}
