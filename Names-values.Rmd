...
1.  Given the following data frame, how do I create a new column called "3"
    ...
    ::: answer
    The disadvantage is that we need to use backticks to wrap the variable names.
    ```{r}
    df$`3` <- df$`1` + df$`2`
    df
    ```
...
1.  In the following code, how much memory does `y` occupy?
    ...
    pryr::object_size(y)
    ```
    **Update:** It looks like [`pryr`](https://github.com/hadley/pryr) is superseded.
    :::
...
1.  On which line does `a` get copied in the following example?
    ...
    ::: answer
    `a` gets copied when `b` is modified.
    ```{r, eval = FALSE}
    b[[1]] <- 10
    ```
    :::
...
1.  Explain the relationship between `a`, `b`, `c` and `d` in the following 
    ...
    ::: answer
    The first three names point to the same identifier. The fourth name, `d`, points to a newly created object.
...
1.  By default, base R data import functions, like `read.csv()`, will 
    ...
    ::: answer
    This is problematic because we risk losing important information stored in the variable names. This behaviour can be turned off by setting `check.names = FALSE`.
...
1.  What rules does `make.names()` use to convert non-syntactic names into
    syntactic ones?
    ::: answer
    `make.names()` prepends the character `"X"` to non-syntactic names, and appends a dot `"."` to reserved words.
    ```{r}
    make.names(c("good", "_bad", "function"))
    ```
    :::
...
1.  I slightly simplified the rules that govern syntactic names. Why is `.123e1`
    ...
    ::: answer
    `.123e1` is not a syntactically valid name because it starts with a dot and is followed by the number.
    :::
...
::: answer
Basically, it is more costly in terms of memory to modify rows.
:::
...
1.  Why is `tracemem(1:10)` not useful?
    ...
    ::: answer
    `tracemem(1:10)` is not useful because the `1:10` object is always given a new identifier unless it has been bound to a name. There is also no way of modifying the `1:10` object without binding it to a name.
    ```{r}
    obj_addr(1:10)
    obj_addr(1:10)
...
1.  Explain why `tracemem()` shows two copies when you run this code.
    ...
    ::: answer
    This is what is returned by the above chunk:
    ```{r ref.label = "two-copies", results = TRUE}
    ```
    
    Two copies are generated because the object associated with `x` changes type. When created, the object is of type integer. Once the third element is replaced, the object is coerced to type double because the replacement value `4` is an atomic double.
    ...
    Absent type coercion, only one copy is generated to account for the change in the third element of the vector.
    ```{r}
    x <- c(1L, 2L, 3L)
    tracemem(x)
...
1.  What happens when you run this code?
    ...
    ::: answer
    The resulting object is a list with two elements: a numeric vector, and a list which calls the first numeric vector.
    ```{r}
    x
    ```
    
    Only one copy of the object is made. The copy occurs because we add a new element to the list.
    ```{r}
    x <- list(1:10)
    tracemem(x)
    ...
    Note that the vector `1:10` is never copied. The example below shows this more clearly.
...
1.  In the following example, why are `object.size(y)` and `obj_size(y)`
    ...
    ::: answer
    Unlike `lobstr::obj_size()`, `object.size()` does not detect if elements of a list are shared. This results in radically different estimates for `y` since it is made of 100 repated atomic vectors.
...
1.  Take the following list. Why is its size somewhat misleading?
    ...
    ::: answer
    According to the [solutions manual](https://advanced-r-solutions.rbind.io/names-and-values.html#object-size), the size is misleading because the list contains functions from the `base` and `stats` packages that are built-in to R. 
    
    `r emo::ji("confused")` I don't really understand this logic. Does it make sense to calculate the size of objects that are already loaded by default in every R session? Probably not. But in that case I wouldn't say that the result is _misleading_, it is just futile.
    
    `r emo::ji("gem")` While reading the documentation I found a nice example of _splatting_ with `!!!`.
    
    ```{r}
    obj_sizes(mean, sd, var)
...
1.  Predict the output of the following code:
    ...
    ::: answer
    In the beginning, the size of the `b` is almost the same as that of `a` since the list is made up of two shared values.
    
    When the second element of the list is modified, a new copy is created, which (almost) doubles the size of `b`. This makse sense since the list is now made up of two different vectors.
    
    When the first element of the list is modified, the size of `b` remains the same as the previous step. Although the two elements of the vector are back to being the same, R doesn't recognise them as a shared value. This also explains why in total, `a` and `b` have the size of three distinct values.
    ```{r, ref.label = "ab-sizes"}
    ```
    :::
...
1.  Explain why the following code doesn't create a circular list.
    ...
    ::: answer
    According to the [solutions manual](https://advanced-r-solutions.rbind.io/names-and-values.html#modify-in-place), this is because of copy-on-modify prevents the creation of a circular list.
    
    ```{r}
    x <- list()
    
    # at first, x points to the address of the empty list
    tracemem(x)
    ...
    # but the empty list continues to point to the old address
    obj_addr(x[[1]])
    
    # same thing can be seen with ref()
    ref(x)
    ```
    :::
...
1.  Wrap the two methods for subtracting medians into two functions, then
    ...
    ::: answer
    In the code below, I create three functions: one that generates random data from a given number of columns and two that implement the median subtraction. Note that I also make sure that both median functions output a `data.frame` object. 
    ```{r, error = TRUE}
    generate_data <- function(ncol) {
      data.frame(matrix(runif(ncol * 1e4), ncol = ncol))
    }
    
    df_median <- function(x, medians) {
      for (i in seq_along(medians)) {
        x[[i]] <- x[[i]] - medians[[i]]
      }
      x
    ...
    I use `bench::press()` to evaluate the performance of the methods as the number of columns grows, and store the results in `results`.
    
    ```{r, eval = FALSE}
    results <- bench::press(
      ncol = c(5, 50, 100, 200, 400, 600, 800, 1000, 1500),
      {
        x <- generate_data(ncol)
        medians <- vapply(x, median, numeric(1))
        bench::mark(
          "Data Frame" = df_median(x, medians),
          "List"       = ls_median(x, medians),
          filter_gc    = FALSE # keep iterations where the gc was activated
        )
      }
    )
    ```
    ...
    # I had problems saving the full df, so I kept only what I needed
    results2 <- dplyr::select(results, ncol, median, expression)
    saveRDS(results2, "solution-files/results.rds")
    ```
    
    ```{r, include = FALSE}
    library(bench)
    results <- readRDS("solution-files/results.rds")
    ```
    
    It looks like the performance of both approaches is relatively similar for less that 500 columns. After that point, the median computation time begins to grow exponentially for the data frame approach.
    
    ```{r}
    library(ggplot2)
    p <- ggplot(results, aes(ncol, median, color = as.character(expression))) +
      geom_point(size = 2) +
    ...
    For a small number of `ncols`, we tend to see the data frame method outperforming the list method. This is because the `data.frame -> list -> data.frame` conversion imposes a minor performance penalty in `ls_median()`.
    
    ```{r}
    p + coord_cartesian(xlim = c(0, 500))
    ```
    :::
...
1.  What happens if you attempt to use `tracemem()` on an environment?
    ...
    ::: answer
    According to the documentation, it is not useful to trace environments are these are not duplicated (i.e. they are modified in place).
    ```{r, error = TRUE}
    e1 <- rlang::env(a = 1, b = 2, c = 3)
    tracemem(e1)
    ```
